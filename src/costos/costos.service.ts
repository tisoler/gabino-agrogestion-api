    ...
    if (scope === "global") {
      ...
      if (!isAdmin) {
        ...
        if (ids.length === 0) {
          ...
          query.andWhere(
            "(costo.id_empresa IS NULL OR costo.id_empresa IN (:...ids))",
            { ids },
          );
        }
      }
    ...
    if (!isAdmin) {
      if (costo.idEmpresa === null) {
        throw new ForbiddenException(
          "No tiene permisos para editar un costo global",
        );
      }
      if (!userEmpresas.includes(costo.idEmpresa)) {
        throw new ForbiddenException(
          "No tiene permisos para editar un costo de otra empresa",
        );
      }
    }
