...
export class CampaniasService {
  constructor(
    ...
    @InjectRepository(CampaniaCosto) private campaniaCostoRepo: Repository<CampaniaCosto>,
  ...
  async findAll(user: any, filters: FindCampaniasFilters = {}) {
    ...
    return cabeceras.map<CampaniaListItem>((c) => {
      ...
      const r = calcularResultados({
        supSembrada,
        supCosechada,
        prodNetaTotalQq: c.prodNetaTotalQq != null ? Number(c.prodNetaTotalQq) : 0,
        precioXQq: c.precioXQq != null ? Number(c.precioXQq) : 0,
        comercializacionPct: c.comercializacionPct != null ? Number(c.comercializacionPct) : 0,
  ...
  // Detalles: LABORES
  // ---------------------------------------------------------------------------
  async addLabor(campaniaId: number, dto: CreateCampaniaDetalleLaborDto, user: any) {
  ...
  async updateLabor(campaniaId: number, detalleId: number, dto: UpdateCampaniaDetalleLaborDto, user: any) {
    const campania = await this.campaniaOrThrow(campaniaId, user);
    const detalle = await this.campaniaLaborRepo.findOne({ where: { id: detalleId, idCampania: campaniaId } });
  ...
  // Detalles: INSUMOS
  // ---------------------------------------------------------------------------
  async addInsumo(campaniaId: number, dto: CreateCampaniaDetalleInsumoDto, user: any) {
    const campania = await this.campaniaOrThrow(campaniaId, user);
  ...
  async updateInsumo(campaniaId: number, detalleId: number, dto: UpdateCampaniaDetalleInsumoDto, user: any) {
    const campania = await this.campaniaOrThrow(campaniaId, user);
    const detalle = await this.campaniaInsumoRepo.findOne({ where: { id: detalleId, idCampania: campaniaId } });
    if (!detalle) throw new NotFoundException('Detalle de insumo no encontrado');
  ...
  // ---------------------------------------------------------------------------
  async addCosto(campaniaId: number, dto: CreateCampaniaDetalleCostoDto, user: any) {
    const campania = await this.campaniaOrThrow(campaniaId, user);
  ...
  async updateCosto(campaniaId: number, detalleId: number, dto: UpdateCampaniaDetalleCostoDto, user: any) {
    const campania = await this.campaniaOrThrow(campaniaId, user);
    const detalle = await this.campaniaCostoRepo.findOne({ where: { id: detalleId, idCampania: campaniaId } });
  ...
  private async assertCampaniaAcceso(campania: Campania, user: any, accion: string) {
    ...
    if (!userEmpresas.includes(lote.idEmpresa)) {
      throw new ForbiddenException(`No tiene permisos para ${accion} esta campaña`);
    }
  }
  ...
  private async assertVariedad(idVariedad: number, idCultivo: number, user: any) {
    const variedad = await this.variedadRepo.findOne({ where: { id: idVariedad } });
    if (!variedad) throw new BadRequestException('La variedad indicada no existe');
   ...
   */
  private async assertLaborEnScope(idLabor: number, idEmpresaLote: number, user: any) {
  ...
  private async assertInsumoEnScope(idInsumo: number, idEmpresaLote: number, user: any) {
  ...
  private async assertCostoEnScope(idCosto: number, idEmpresaLote: number, user: any) {
